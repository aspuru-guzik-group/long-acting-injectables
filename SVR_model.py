...
# set support vector regressor to SVR
SVR=SVR()
...
param_distribs={
    ...
    'gamma':['scale', 'auto'],
    'C':[0.1, 0.5, 1, 2],
    'epsilon':[0.001, 0.01, 0.1, 0.15, 0.2],
    'shrinking': [True, False],
}

# run rnd_search applying logo for each of the 250 interation and scoring based on mean absolute error
rnd_search_cv = RandomizedSearchCV(SVR, param_distribs, n_iter=250,verbose = 3, cv=logo, scoring='neg_mean_absolute_error')
...
# train SVR based on best paramaters identified during rnd_search and return model structure
regressor=rnd_search_cv.best_estimator_
regressor.fit(X=X,y=Y)

# save trained SVR model to pickle file
