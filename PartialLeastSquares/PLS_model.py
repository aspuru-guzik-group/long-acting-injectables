...
# set partial least squares regressor to PLSR
PLSR=PLSRegression()
...
# run rnd_search applying logo for each of the 10 interation and scoring based on mean absolute error
rnd_search_cv = RandomizedSearchCV(PLSR, param_distribs, n_iter=10,verbose = 3, cv=logo, scoring='neg_mean_absolute_error')
...
# train PLS based on best paramaters identified during rnd_search and return model structure
regressor=rnd_search_cv.best_estimator_
regressor.fit(X,Y)

# saved trained PLS model to pickle file
Pkl_Filename = "PLS.pkl"  
with open(Pkl_Filename, 'wb') as file:  
    pickle.dump(regressor, file)

# Load trained model and return structre
with open("PLS.pkl", 'rb') as file:  
